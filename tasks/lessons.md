...
## 2026-08-04 リリース準備で、CLI の出力形式を確認せず監視スクリプトを組み、掲載文の真偽判定で主語を読み違えた

### 17. ポーリング用スクリプトに CLI を組み込む前に、1 回だけ手で実行して「フラグの実在」と「値の表記」を確認する
- **何があったか**: EAS ビルドの完了監視をバックグラウンドで仕込む際、`eas build:view --json --non-interactive` と書き、ステータスを `finished` / `errored` の小文字で分岐させた。実際には (1) `build:view` に `--non-interactive` フラグは**存在せず**コマンドごと失敗し、(2) ステータスの実値は `IN_PROGRESS` / `FINISHED` の**大文字**、(3) stdout にスピナー行と DeprecationWarning が混入して `JSON.parse` が落ちる、の 3 点が同時に外れていた。`status=unknown` を 8 回・16 分間出力し続け、ビルドの実態を一切掴めていなかった。
- **原因**: 直前に成功した `build:list --non-interactive` からフラグを類推し、ステータス表記も推測で決めた。**同じ CLI でもサブコマンドごとにフラグ集合は異なる**。加えて「バックグラウンドに投げた」ことで失敗の可視性が下がり、発見が遅れた。
- **ルール**: ループ／バックグラウンドに CLI を埋め込む前に、必ず**フォアグラウンドで 1 回実行して生出力を目視**する。確認するのは (1) フラグが実在するか（`--help` か実行で確認。他サブコマンドからの類推禁止）、(2) 分岐に使う値の実際の表記（大小文字・語彙）、(3) stdout に JSON 以外が混ざらないか。`--json` を謳う CLI でもスピナーや警告が混ざる前提で、`s.slice(s.indexOf("{"), s.lastIndexOf("}")+1)` のように JSON 部分を切り出し、`NODE_NO_WARNINGS=1` を付ける。パース失敗時に `unknown` へ握り潰す実装は**沈黙する故障**になるので、初回パース失敗は即座にエラー終了させる。
- **関連ファイル**: `apps/mobile/eas.json`

### 18. ストア掲載文・規約文の「将来偽になるか」は、文全体ではなく主語のスコープで判定する
- **何があったか**: 将来のディスプレイ広告導入に備えて掲載文を精査した際、「入力したアジェンダは端末内にのみ保存され、外部に送信されません」も広告 ID の送信により偽になると誤って断定した。実際には主語が「アジェンダ」に限定されており、広告 SDK が広告 ID を送っても**この文は真のまま**である。一方「広告や解析ツールは含まれていません」はアプリ全体への断定で、導入と同時に偽になる。
- **ルール**: 掲載文・プライバシーポリシー等の「将来の陳腐化」を評価するときは、文ごとに**主語が何に限定されているか**を先に取り出してから真偽を判定する。アプリ全体を主語にした断定（「〜は含まれていません」）は寿命が短く、対象を絞った事実（「〜は送信されません」）や時点を明示した限定（「現時点で〜」）は SDK 追加後も生き残る。将来変わりうる約束は、書く時点で主語か時点を絞っておく。
- **副次の学び**: Play Console の「広告の有無」申告は**将来の予定ではなく、いま審査に出すバイナリの実態**を問うもの。後から「なし→あり」へ再審査なしで変更できるため、先回りして「あり」と申告する利得は無く、ストアに広告バッジが付く不利益だけが残る。掲載文（審査対象・反映に日数）と申告値（即時変更可）は**変更コストが違う**ので、前者だけ先回りして直しておくのが正しい。
- **関連ファイル**: `docs/09-play-store-listing.md`

## 2026-08-07 タグ起動のリリースワークフローで、外部 CLI の前提を推測して 3 回失敗した

### 19. 「クラウドでビルドする CLI」でも、ローカルの依存解決を必要とする前段があるか実行して確かめる
- **何があったか**: `release.yml` を組む際「`eas build` はビルド処理を EAS のクラウドで行うのだから、runner 側に `node_modules` は不要」と判断し、`pnpm install` を入れずにビルドステップを書いた。初回タグ push で `Failed to resolve plugin for module "expo-router" ... Do you have node modules installed?` により即失敗。EAS CLI はソースをアップロードする**前にローカルで app config を評価**しており、`app.json` の `plugins: ["expo-router"]` を解決するのに依存が要る。
- **原因**: 「重い処理がどこで走るか」から「何が不要か」を演繹した。実際に必要なのは処理の場所ではなく、**CLI がローカルで何を読むか**である。`app.config.js` / plugins / monorepo の workspace 解決など、設定評価はほぼ必ずローカルで起きる。
- **ルール**: CI に外部 CLI を組み込むときは「この CLI はローカルで何を読むか」を基準に前段を決める。設定ファイルにプラグイン・プリセット・拡張の**モジュール参照**が 1 つでもあれば依存インストールは要ると見なす。省略の判断は、省いた構成を 1 回走らせて確認できるまで確定させない。
- **関連ファイル**: `.github/workflows/release.yml`

### 20. 修正 PR に「揃える」目的の変更を混ぜない。動いていたステップを壊す
- **何があったか**: 19 の修正（`pnpm install` 追加）と同じ PR で、`expo/expo-github-action` の `packager` を `npm` から `pnpm` へ「リポジトリのパッケージマネージャに揃える」目的で変更した。結果、`pnpm add eas-cli` が `[ERR_PNPM_IGNORED_BUILDS] Ignored build scripts: dtrace-provider@0.8.8` で終了コード 1 を返し、**eas-cli のインストールは成功している**（ログに `+ eas-cli 21.6.0`）のにステップが落ちた。直前の実行では `packager: npm` でこのステップは success しており、自分で壊したことになる。リリースが 1 サイクル余計に遅れた。
- **原因**: `packager` が何を指すかを確認せず「パッケージマネージャは統一すべき」という一般則を当てた。実際には `packager` は **eas-cli 自体の入手手段**にすぎず、プロジェクトの依存解決には関与しない。統一する意味が無い値だった。
- **ルール**: 失敗を修正する PR には、失敗の原因に直接対応する変更**だけ**を入れる。「揃える」「統一する」は動作に効かない場合が多く、効く場合はそれ自体が別の変更である。特に**直前の実行で success していたステップに触る**のは、その成功を捨てる行為だと自覚する。設定値を変える前に、その値が何を制御しているのかを一次情報で確認する。
- **副次の学び**: pnpm はビルドスクリプトを既定で実行せず、その事実を**終了コード 1** で通知する。ログ本文が成功を示していても失敗扱いになるため、CI では「インストールは通ったのに赤い」という読み違えを招く。
- **関連ファイル**: `.github/workflows/release.yml`

### 21. 外部サービスの資格情報は、ワークフローを書く前に「登録済みか」を実行して確かめる
- **何があったか**: `eas.json` の `submit.production` が空であることから「EAS 側に Google サービスアカウント鍵が保存済みかは未確認」と気づき、ユーザーにも伝えていたが、確認を促すに留めてリリースを開始した。3 回目の実行でビルド（約 14 分）まで到達した後、`Google Service Account Keys cannot be set up in --non-interactive mode.` で提出が失敗した。鍵は未登録だった。
- **ルール**: 外部サービスへ提出・デプロイするワークフローでは、**資格情報の登録状況を先に確定させてから**起動する。懸念を口頭で伝えるだけでは確認したことにならない。非対話で読めない資格情報（対話 CLI でしか一覧できない類）は、ユーザーに「登録済みか」を yes/no で答えられる形で問い、未確認のまま長時間ステップの後段に置かない。
- **副次の学び**: 一方で、`main` への反映と GitHub Release 作成を**提出成功の後**に配置した設計は正しく働いた。3 回の失敗を通じて `main` は `Initial commit` のまま、GitHub Release も未作成で、タグを打ち直すだけで何度でもやり直せた。外部反映を伴うワークフローは、**不可逆な操作ほど後段に置く**。
- **関連ファイル**: `.github/workflows/release.yml`、`apps/mobile/eas.json`
